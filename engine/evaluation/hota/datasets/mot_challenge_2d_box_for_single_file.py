...
class MotChallenge2DBox_for_single_file(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'GT_file': '',
            'TRACKER_FILE': '',
            'OUTPUT_FOLDER': '',  # Where to save eval results (if None, same as TRACKERS_FOLDER)
    ...
    def __init__(self, config=None):
        ...
        self.tracker_input_file = self.config['TRACKER_FILE']
        self.tracker_list = [self.config['display_name_in_plot']]
        self.seq_list = [self.config['display_name_in_plot']]
        self.output_sub_fol = self.config['OUTPUT_SUB_FOLDER']

        # Get classes to eval
        self.valid_classes = ['pepper']
        self.class_list = [cls.lower() if cls.lower() in self.valid_classes else None
                           for cls in self.config['CLASSES_TO_EVAL']]
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class. Only pedestrian class is valid.')
        self.class_name_to_class_id = {'pepper': 1,'other':2}
        self.valid_class_numbers = list(self.class_name_to_class_id.values())
        self.tracker_to_disp = dict(zip(self.tracker_list, self.tracker_list))


        # Get sequences to eval and check gt files exist
        # self.seq_lengths = self.config["total_frame_num"]
        self.unique_frame_names = self.config['unique_frame_names']
        self.seq_lengths = len( self.unique_frame_names )

        # Check gt files exist

        curr_file = self.config["GT_file"]
        if not os.path.isfile(curr_file):
            print('GT file not found ' + curr_file)
            raise TrackEvalException('GT file not found for sequence: ' + self.config["GT_file"])

        # Get trackers to eval

        curr_file = self.tracker_input_file
        if not os.path.isfile(curr_file):
            print('Tracker file not found: ' + curr_file)
            raise TrackEvalException(
                'Tracker file not found: ' + self.tracker_input_file + '/' + os.path.basename(
                    curr_file))
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        for t in range(num_timesteps):
            # time_key = str(t+1)
            time_key = self.unique_frame_names[t]
