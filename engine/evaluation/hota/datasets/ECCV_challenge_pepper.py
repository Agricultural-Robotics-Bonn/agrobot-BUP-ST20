...
class ECCVChallenge2DBox(_BaseDataset):
    ...
    def __init__( self, config=None ):
      ...
      self.seq_list, self.seq_lengths, self.seq_frames = self._get_seq_info()
    ...
    def _get_seq_info(self):
        ...
        seq_lengths = {}
        seq_frames = {}
        for k, v in self.config["SEQ_INFO"].items():
          seq_lengths[k] = len( v )
          seq_frames[k] = v
        return seq_list, seq_lengths, seq_frames
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        if is_gt:
          file = os.path.join( self.gt_fol, seq, 'gt.txt' )
        else:
          file = os.path.join( self.tracker_fol, seq+'.txt')
        read_data, ignore_data = self._load_simple_text_file( file, is_zipped=False, zip_file=None )
        ...
        for t in range(num_timesteps):
            # time_key = str(t+1)
            time_key = self.seq_frames[seq][t]
